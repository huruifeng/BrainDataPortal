...
function XDatasetsView() {
  ...
  // Track which genes have been fetched for each dataset
  const [fetchedGenes, setFetchedGenes] = useState({})

  // Track ongoing data fetching operations to prevent duplicate requests
  ...
  // Check if a gene has been fetched for a dataset and sample
  const isGeneFetched = (datasetId, sample, gene) => {
    if (!fetchedGenes[datasetId]) return false
    if (!fetchedGenes[datasetId][sample]) return false
    return fetchedGenes[datasetId][sample].includes(gene)
  }
  ...
  const loadDatasetData = async (index) => {
    ...
    const pendingKey = `metadata-${datasetId}`
    if (pendingFetchesRef.current[pendingKey]) {
    ...
    // Mark as pending
    pendingFetchesRef.current[pendingKey] = true
    ...
    try {
      ...
      // Clear pending flag
      delete pendingFetchesRef.current[pendingKey]
    }
  }
  ...
  const loadSampleData = async (index, sample) => {
    ...
    // Create a unique key for this dataset-sample combination
    const pendingKey = `sample-${datasetId}-${sample}`

    // Skip if already loading this dataset-sample combination
    if (pendingFetchesRef.current[pendingKey]) {
      console.log(`Already loading data for dataset ${datasetId} sample ${sample}, skipping duplicate request`)
      return
    }
    ...
    try {
      ...
      // Skip if there are no gene features to fetch
      if (geneFeatures.length === 0) {
        console.log(`No gene features to fetch for dataset ${datasetId} sample ${sample}`)

        // Still need to fetch metadata for this sample
        await setDataset(datasetId)
      ...
      // Filter out genes that have already been fetched
      const genesToFetch = geneFeatures.filter((gene) => !isGeneFetched(datasetId, sample, gene))

      if (genesToFetch.length === 0) {
        console.log(`All genes already fetched for dataset ${datasetId} sample ${sample}`)
      ...
      console.log(
        `Fetching ${genesToFetch.length} genes for dataset ${datasetId} sample ${sample}: ${genesToFetch.join(", ")}`,
      )
      ...
      // Update the list of fetched genes
      setFetchedGenes((prev) => ({
        ...prev,
        [datasetId]: {
          ...prev[datasetId],
          [sample]: [...(prev[datasetId]?.[sample] || []), ...genesToFetch],
        },
      }))
  ...
  useEffect(() => {
    ...
    const processSampleData = async () => {
      for (const [index, dataset] of datasetsWithSamples.entries()) {
        // Find the index of this dataset in the original datasets array
        const originalIndex = datasets.findIndex((d) => d.id === dataset.id && d.sample === dataset.sample)
        if (originalIndex !== -1) {
  ...
  // Reset fetched genes when features change
  useEffect(() => {
    // When features change for a dataset, we need to reset the fetched genes
    // to ensure we fetch any new genes that were added
    const resetFetchedGenes = () => {
      const newFetchedGenes = { ...fetchedGenes }

      datasets.forEach((dataset) => {
        if (dataset.id && dataset.sample) {
          // Reset the fetched genes for this dataset and sample
          if (newFetchedGenes[dataset.id] && newFetchedGenes[dataset.id][dataset.sample]) {
            newFetchedGenes[dataset.id][dataset.sample] = []
          }
        }
      })

      setFetchedGenes(newFetchedGenes)
    }

    resetFetchedGenes()
  }, [datasets.map((d) => d.features.join(",")).join("|")])
    ...
    if (newPlotType !== oldPlotType && dataset.id) {
      ...
      if ((newPlotType === "umap" || newPlotType === "both") && !plotData[datasetId]?.umapdata) {
        ...
        fetchUMAPData(datasetId).then(() => {
          ...
          if (dataset.sample) {
            loadSampleData(index, dataset.sample)
          }
        })
      }
  ...
  const handleRefreshPlots = () => {
    // Reset fetched genes to force a refresh
    setFetchedGenes({})
    ...
    // Reload data for all datasets
    for (const [i, dataset] of datasets.entries()) {
      if (dataset.id && dataset.sample) {
        loadSampleData(i, dataset.sample)
      }
    }
  }
  ...
  const handleRefreshDataset = (index) => {
    ...
    if (dataset.id && dataset.sample) {
      // Reset fetched genes for this dataset
      setFetchedGenes((prev) => {
        const newFetchedGenes = { ...prev }
        if (newFetchedGenes[dataset.id]) {
          newFetchedGenes[dataset.id][dataset.sample] = []
        }
        return newFetchedGenes
      })
      ...
      // Clear pending fetches for this dataset
      Object.keys(pendingFetchesRef.current).forEach((key) => {
        if (key.includes(`-${dataset.id}-`)) {
          delete pendingFetchesRef.current[key]
        }
      })
  ...
  console.log("===fetchedGenes", fetchedGenes)
                            ...
                            {(effectivePlotType === "visium" || effectivePlotType === "both") && (
                              <Box className="feature-plot">
                                ...
                                plotData[dataset.id]["metadict"]  ? (
                                  dataset.sample === "all" ? (<Typography variant="body2" color="text.secondary">Please select a specific sample.</Typography>) : (
                                    (() => {
                                      return (
                                  ...
                                  <Box sx={{display: "flex", justifyContent: "center", alignItems: "center", height: "100%", bgcolor: "#f5f5f5", borderRadius: 1,}}>
                                    <Typography variant="body2" color="text.secondary">Loading Visium data...</Typography>
                                  </Box>
                                )}
                              </Box>
                            )}
