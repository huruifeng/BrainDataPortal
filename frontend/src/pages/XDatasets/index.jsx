    ...
    datasets.forEach((dataset) => {
      if (dataset.id && dataset.id !== "") {
        if (!(dataset.id in plotData)) {
          ...
          dataset.genelist = geneList

          fetchSampleList(dataset.id)
          dataset.samplelist = sampleList

          fetchMetaList(dataset.id)
          dataset.metaList = metaList
          // If sample is selected, fetch its data
          if (dataset.sample) {
          ...
          {datasets.map((dataset, index) => (
            ...
            >
              ...
              <Paper elevation={1} sx={{ p: 1, mb: 2 }}>
                ...
                >
                  <Typography variant="subtitle2">Dataset {index + 1}</Typography>
                  <Box>
                    {datasets.length > 2 && (
                ...
                <Autocomplete
                  size="small"
                  options={datasetRecords || []}
                  value={dataset.id}
                  onChange={(event, newValue) => handleDatasetChange(index, newValue)}
                  renderInput={(params) => (
                    <TextField {...params} label="Select dataset" variant="outlined" margin="dense" />
                  )}
                />
                ...
                <Autocomplete
                  ...
                  disabled={!dataset.id}
                  renderInput={(params) => (
                    <TextField {...params} label="Select sample" variant="outlined" margin="dense" />
                  )}
                />
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} label="Select features" variant="outlined" margin="dense" />
                  )}
                />
              </Paper>
